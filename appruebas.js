$(function () {
	...
	$.when($.getJSON(peticion, function obtener_fotos(info) { // Obtenemos las fotos públicas de los contactos
		var i;
		for (i=0;i<info.photos.photo.length;i++) {
			var item = {url: "", owner: "", name: "", date_taken: "", date_upload: ""}; // Estructura de datos
			var aux = info.photos.photo[i]; // Obtenemos una a una
			item.url = 'https://farm'+aux.farm+".staticflickr.com/"+aux.server +'/'+aux.id+'_'+aux.secret+'_m.jpg'; // URL
